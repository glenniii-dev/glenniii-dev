        ...
        {items.map((item, index) => (
          <li key={item.id} ref={(el) => { itemsRef.current[index] = el}} className="list-item opacity-0f"
            ...
            <Link 
              ...
              target="_blank"
              href={contentType === "blog" ? `/blog/${item.id}` : `${item.link}`} 
              className="flex flex-col justify-between border-t border-t-slate-100 py-10 md:flex-row text-slate-100">
              <div className="flex flex-col">
                <span className={`text-3xl font-bold ${contentType === "blog" ? "text-slate-200" : "text-yellow-400"}`}>{item.title}</span>
                <div className="flex gap-3 text-lg max-w-4xl">
                  {item.tags && item.tags.map((tag, tagIndex) => ( 
                    <span key={tagIndex} className="text-yellow-400 font-bold">{tag}</span>
                  ))}
