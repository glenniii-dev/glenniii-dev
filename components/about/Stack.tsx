  ...
  return (
    <section>
      <div className="w-full max-w-7xl mx-auto items-center px-4 py-10 md:px-6 md:py-14 lg:py-16 space-y-20">
        <h1 className="col-start-1 text-6xl md:text-8xl font-bold mb-8">What I Use</h1>
      </div>
      ...
      <div className="block lg:hidden  max-w-screen overflow-hidden">
        {stack.map((item, index) => (
          <div key={index} aria-label={item.tech || undefined} className="tech-row mb-8 flex items-center justify-center gap-4 text-slate-700">
            {Array.from({ length: 15 }, (_, index) => (
              <React.Fragment key={index}>
                <span className="text-item text-5xl font-extrabold uppercase tracking-tighter" style={{ color: index === 7 && item.color ? item.color : "inherit" }}>
                  {item.tech}
                </span>
